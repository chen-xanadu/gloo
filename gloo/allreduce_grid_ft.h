...
template<typename T>
class AllreduceGridFT : public Algorithm {
 public:
  AllreduceGridFT(
    ...
    auto notificationSlot = this->context_->nextSlot(1);
    sendNotificationBuf_ =
        leftPair->createSendBuffer(notificationSlot, &msg_, sizeof(msg_));
    recvNotificationBuf_ =
        rightPair->createRecvBuffer(notificationSlot, &msg_, sizeof(msg_));
    ...
    for (int i = 0; i < groups_ - 1; i++) {
      ...
      sendCopyNotificationBuf_.push_back(
          copyPair1->createSendBuffer(slot, &msg_, sizeof(msg_)));
      auto &copyPair2 = this->getPair(copyPairs_[groups - 2 - i]);
      recvCopyNotificationBuf_.push_back(
          copyPair2->createRecvBuffer(slot, &msg_, sizeof(msg_)));
    }

  }

  virtual ~AllreduceGridFT() {
    for (int i = 0; i < 2; i++) {
      if (inbox_[i] != nullptr) {
        free(inbox_[i]);
      }
 ...
 protected:
  ...
  void repairRow(int repairGroupId) {
    ...
    sendCopyNotificationBuf_[failedSendIdx] = backupPair->createSendBuffer(slot, &round_, sizeof(msg_));
    slot = slotOffset + failedRecvIdx;
    recvCopyNotificationBuf_[failedRecvIdx] = backupPair->createRecvBuffer(slot, &backupPairRound_, sizeof(msg_));
  ...
  void recoverLeftPair() {
    ...
    for (int i = 0; i < groups_ - 1; i++) {
      ...
      backupSendCopyNotificationBuf_.push_back(
          copyPair1->createSendBuffer(slot, &round_, sizeof(msg_)));
      auto &copyPair2 = this->getPair(backupCopyPairRank_[groups_ - 2 - i]);
      backupRecvCopyNotificationBuf_.push_back(
          copyPair2->createRecvBuffer(slot, &backupRowRound_[i], sizeof(msg_)));
    }
    ...
    auto syncRoundBuf = leftPair->createRecvBuffer(notificationSlot + 2, &leftPairRound_, sizeof(leftPairRound_));
  ...
  void recoverRightPair() {
    ...
    auto syncRoundBuffer = rightPair->createSendBuffer(notificationSlot + 2, &round_, sizeof(round_));
